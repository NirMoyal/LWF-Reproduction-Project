...
According to the scope agreed with the course instructor, the goal was to reproduce the main qualitative trend shown in **Table 1** of the original paper, using two ImageNet-based experiments from that table.
...
The original paper includes additional task pairs, datasets, architectures, and baselines. These were **outside the agreed project scope**. In particular, this project was not required to reproduce the full paper, all of Table 1, or every method reported by Li and Hoiem. The purpose was to demonstrate the same main behavior:
...
└── Results/
    ...
    ├── Results table.pdf
    ├── scenes_per_seed_results.csv
    ├── scenes_report_table.csv
    └── scenes_summary_results.csv
```
...
### `Results/`
Contains exported result tables, raw CSV result files, and comparison figures used in the report.

The raw CSV files are included so the reported results can be inspected directly:

| File | Description |
|---|---|
| `cub_single_seed_results.csv` | Raw single-seed results for the ImageNet → CUB experiment. |
| `cub_report_table.csv` | Report-ready CUB results table. |
| `scenes_per_seed_results.csv` | Raw per-seed results for the ImageNet → MIT Indoor Scenes experiment. |
| `scenes_summary_results.csv` | Summary results for the Scenes experiment. |
| `scenes_report_table.csv` | Report-ready Scenes results table. |

The PDF files in this directory contain the visual result tables and comparison figures used in the final report.
...
The raw CSV result files used to produce these tables are available in the `Results/` directory.
