...
This project reproduces **only two experiments** from the original LwF paper, according to the scope agreed with the course instructor:
...
The original paper contains additional experiments, datasets, and baselines. These were outside the agreed scope of this project.

To keep the reproduction consistent and computationally feasible, the experiments were executed using:
...
The use of one seed and 80 ImageNet validation batches was also agreed with the course instructor.  
...
The project uses a consistent protocol across the two reproduced experiments, as agreed with the course instructor.
...
The ImageNet validation evaluation is limited to the first **80 batches** due to compute limitations and according to the agreed project scope.  
...
- The reproduction scope was agreed with the course instructor and includes only two experiments: ImageNet → CUB-200-2011 and ImageNet → MIT Indoor 67 Scenes.
- Each experiment was executed with one fixed seed (`42`), as agreed with the course instructor.
- The project is a reproduction and educational implementation, not an exact full-scale rerun of all experiments from the original paper.
- ImageNet evaluation is performed on a fixed 80-batch validation subset to reduce runtime.
